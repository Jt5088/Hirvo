...
## Key Rules
- **Sections are the source of truth.** `index.html` uses `<!-- INCLUDE: sections/foo.html -->` markers only — no inline HTML.
...
- **JS files are IIFEs** — `(function() { ... })();` pattern. No `import`/`export`.
- **All design tokens live only in `tokens.css`.** → See `.claude/tokens.md`
